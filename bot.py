...
async def on_member_update(before, after):
    ...
    steam_url = next((r.get('steam_url') for r in recs if r.get('discord_id') == str(after.id)), None)
...
async def find_teammates(interaction, игра: str):
    ...
    mentions = [f"{interaction.guild.get_member(int(uid)).mention} ({hrs}ч)"
...
async def common_games(interaction, user: discord.Member):
    ...
    recs = init_gspread_client().worksheet('Games').get_all_records()
    data = {r['discord_id']: {} for r in recs}
    for r in recs: data[r['discord_id']][r['game_name']] = int(r['playtime'])
    ...
    if not common: return await safe_respond(interaction, content='ℹ️ Общие игры не найдены.', ephemeral=True)
    desc = '\n'.join(f"**{g}** — вы: {h1}ч, {user.display_name}: {h2}ч"
                     for g,h1,h2 in sorted(common, key=lambda x: x[1], reverse=True))
...
async def discount_game_check():
    ...
    for r in fresh: sws.append_row([r['game_link'], r['discount_end']])
    ...
    for item in soup.select('.search_result_row')[:5]:
        discount = item.select_one('.search_discount .discount_pct')
        if not discount or '-100%' not in discount.text: continue
        ...
        if not end_text: continue
        if any(r['game_link'] == link for r in fresh): continue
        if ch: await ch.send(f'🔥 100% скидка: [{item.select_one(".title").text.strip()}]({link}) до {end_text}')
        sws.append_row([link, end_text])

@tasks.loop(hours=24)
async def epic_free_check():
    sh = init_gspread_client(); ews = sh.worksheet('SentEpic')
    rows = ews.get_all_records(); now = datetime.utcnow(); fresh = []
    for r in rows:
        try:
            dt = datetime.fromisoformat(r['offer_end'])
            if dt > now: fresh.append(r)
        except: continue
    ews.clear(); ews.append_row(HEADERS['SentEpic'])
    for r in fresh: ews.append_row([r['game_title'], r['offer_end']])
    ...
    for game in offers:
        ...
        promo_list = promos.get('upcomingPromotionalOffers') or promos.get('promotionalOffers') or []
        if not promo_list: continue
        entry = promo_list[0]
        if 'promotionalOffers' in entry:
            offer = entry['promotionalOffers'][0]
            end_ts = offer.get('endDate')
            try: end = datetime.fromtimestamp(float(end_ts) / 1000)
            except: continue
            title = game.get('title')
            if any(r['game_title'] == title for r in fresh): continue
            if end > now and ch:
...
async def health_check():
    ...
    if ch: await ch.send(f'📊 Память: {mem}%, CPU: {cpu}%')

@bot.event
async def on_ready():
    print(f'Logged in as {bot.user}')
    Thread(target=run_flask, daemon=True).start()
    discount_game_check.start(); epic_free_check.start(); health_check.start()
    await bot.tree.sync()
