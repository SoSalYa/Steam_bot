...
INTENTS.members         = True   # on_member_join, on_member_update
INTENTS.presences       = True   # presence updates for status integration
...
STEAM_GAMES_CACHE     = {}  # steamid -> (timestamp, {game: hours})
CACHE_TTL             = timedelta(minutes=30)
ORIGINAL_NICKNAMES    = {}  # user_id -> original nickname
...
async def on_member_join(member: discord.Member):
    try:
        await member.send(
            'Добро пожаловать! Чтобы пользоваться ботом, привяжите свой профиль Steam: `/привязать_steam <ссылка>`.'
        )
    except:
        pass
...
async def link_steam(interaction: discord.Interaction, steam_url: str):
    ...
    await interaction.followup.send(embed=Embed(title='Подтверждаете профиль?', description=pname),
